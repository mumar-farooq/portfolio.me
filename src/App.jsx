...
const PROJECTS = [
  { tag: 'AI Finance Advisor', title: 'An AI-powered translator that delivers fast and accurate multilingual communication.', bg: '#FBE7E6' },
  { tag: 'AI  Translator', title: 'An intelligent financial assistant that provides personalized insights and smart budgeting recommendations.', bg: '#E4EFEA' },
  { tag:'AI OBJECT DETECTION AND TRACKING', title: 'A real-time computer vision system that detects and identifies objects with high accuracy..', bg: '#EAE7F7' },
];
...
const ProjectCard = ({ project }) => {
  ...
  return (
    <Reveal>
      ...
      >
        <div style={{
          position: 'relative', backgroundImage: `url(${financeBg})`, borderRadius: 12, padding: 20,
          marginBottom: 16, aspectRatio: '4 / 3', display: 'flex', alignItems: 'center', justifyContent: 'center',
          overflow: 'hidden',
        }}>
          <div style={{
            width: '90%', backgroundImage: `url(${transBg})`, borderRadius: 8, padding: '10px 14px',
...
const About = () => (
  <section id="about" style={{ backgroundImage: `url(${aboutBg})`, padding: '90px clamp(20px,5vw,64px)' }}>
    <div style={{ maxWidth: 1180, margin: '0 auto' }}>
...
const Blog = () => (
  <section id="blog" style={{ background: '#fff', padding: '90px clamp(20px,5vw,64px)' }}>
    <div style={{ maxWidth: 1180, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px,1fr))', gap: 24, backgroundImage: `url(${geminiGridBg})` }}>
        {BLOG.map((b, i) => (
          <Reveal key={b.title} delay={i * 0.08}>
            <div style={{ border: `1px solid ${T.line}`, borderRadius: 14, overflow: 'hidden', height: '100%', backgroundImage: `url(${geminiCardBg})` }}>
              <div style={{ aspectRatio: '16/10', background: T.card }} />
              <div style={{ padding: 20, backgroundImage: `url(${geminiDetailsBg})` }}>
